...
func (cfg Config) New(out chan<- Event) (*device, error) {
	...
	// Check for SPI
	if cfg.SPI == nil {
		return nil, ErrBadParameter.With("st7789")
	} else {
		d.spi = cfg.SPI
	}
	...
	// Create a GPIO device with backlight
	if cfg.Backlight == 0 {
		d.blpin = PIN_BACKLIGHT
	} else {
		d.blpin = cfg.Backlight
	}
...
func (d *device) String() string {
	str := "<st7789"
	if d.blpin != 0 {
		str += fmt.Sprint(" backlight=", d.Backlight())
	}
	return str + ">"
}
...
func (d *device) Backlight() bool {
	return d.blstate
}

func (d *device) SetBacklight(v bool) {
	if d.blpin != 0 && d.gpio != nil {
		if v {
			d.gpio.High(d.blpin)
		} else {
			d.gpio.Low(d.blpin)
