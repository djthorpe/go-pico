...
func New(cfg Config, out chan<- Event) *device {
	if device, err := cfg.New(out); err != nil {
		panic(err)
	} else {
		return device
	}
...
func (cfg Config) New(out chan<- Event) (*device, error) {
	...
	if out == nil {
		return nil, ErrBadParameter.With("picodisplay")
	} else {
		d.out = out
	}
	...
	// Create GPIO
	g := gpio.Config{
		Out:   []Pin{PIN_LED_R, PIN_LED_G, PIN_LED_B},
		PWM:   []Pin{PIN_LED_R, PIN_LED_G, PIN_LED_B},
		In:    []Pin{PIN_BUTTON_A, PIN_BUTTON_B, PIN_BUTTON_X, PIN_BUTTON_Y},
		Watch: []Pin{PIN_BUTTON_A, PIN_BUTTON_B, PIN_BUTTON_X, PIN_BUTTON_Y},
	}
	if gpio, err := g.New(d.out); err != nil {
		return nil, err
	} else {
		d.gpio = gpio
	}
...
func (d *device) SetLED(r, g, b bool) {
	d.gpio.Set(PIN_LED_R, r)
	d.gpio.Set(PIN_LED_G, g)
	d.gpio.Set(PIN_LED_B, b)
}
